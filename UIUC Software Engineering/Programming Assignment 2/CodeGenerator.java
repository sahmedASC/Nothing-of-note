...
public class CodeGenerator extends VoidVisitorAdapter
{
	...
	public void visit(ClassOrInterfaceDeclaration n, Object arg) {
		...
		if (!n.isInterface() & !n.isAbstract()) {
			...
			//add if (len > 40) return "OMITTED";

			NameExpr left = new NameExpr("len");
			IntegerLiteralExpr right = new IntegerLiteralExpr(40);
			BinaryExpr binaryExpr = new BinaryExpr(left,right, BinaryExpr.Operator.GREATER);
			ReturnStmt thenStmt = new ReturnStmt(new StringLiteralExpr("OMITTED"));
			ReturnStmt elseStmt = new ReturnStmt(new NameExpr("str"));

			IfStmt ifStmt = new IfStmt(binaryExpr,thenStmt,elseStmt);
