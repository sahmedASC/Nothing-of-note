...
public class TokenQueue {
     ...
     * sequence, will throw an illegal state exception --
    ...
    /**
     * Pulls a balanced string off the queue. E.g. if queue is "(one (two) three) four",
     * (,) will return "one (two) three",
     * and leave " four" on the queue. Unbalanced openers and closers can be quoted (with ' or ")
     * or escaped (with \). Those escapes will be left
     * in the returned string, which is suitable for regexes
     * (where we need to preserve the escape), but unsuitable for
    ...
    public String chompBalanced(char open, char close){
        ...
        if (depth > 0) { Validate.fail("Did not find balanced marker at '" + out + "'");}
        return out;
    }
