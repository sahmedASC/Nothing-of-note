...
void deleteALL(Balloon* &jacob){
    std::set<const Balloon*> reference;
    const Balloon* ref=jacob;
    while (true){
        std::pair<std::set<const Balloon*>::iterator, bool> blah = reference.insert(ref);
        if(blah.second==false){
            break;
        }
        else{
            ref=ref->getRope(0);
    ...
    for (std::set<const Balloon*>::iterator it =reference.begin(); it!=reference.end();++it){
        delete *it;
    }
