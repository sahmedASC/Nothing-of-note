...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Implement this function for Lab 10, Checkpoint 1
    ...
    if(find(key) == end()){//the key is not found in the hash table
  ...
  iterator begin() {
    ...
    // Implement this function for Lab 10, Checkpoint 2, Part 1
...
private:
  ...
  void resize_table(unsigned int new_size) {
    std::vector<std::list<KeyType>> newTable(new_size);
    for (unsigned int i=0; i<m_table.size(); ++i) {
      for (hash_list_itr p = m_table[i].begin(); p != m_table[i].end(); ++p){
