...
void StudentTests() {
  ...
  // =======================================================
    ...
    //push back a row of int
    std::vector<int> new_row;
    new_row.push_back(69);
    new_row.push_back(-420);
    new_row.push_back(-1337);
    new_row.push_back(1001);
    new_row.push_back(58008);
    t.push_back_row(new_row);
    assert (t.numRows() == 3);
    assert (t.numColumns() == 5);
    t.print();
    ...
    another.set(1,2,133337);

    assert (t.get(1,2) != another.get(1,2));
    std::vector<int> new_row2;
    new_row2.push_back(314);
    new_row2.push_back(70);
    new_row2.push_back(27818);
    new_row2.push_back(1414);
    new_row2.push_back(88888);
    another.push_back_row(new_row2);
    assert (t.numRows()+1 == another.numRows());
    assert (t.numColumns() == another.numColumns());
    t.print();
...
//---------------------------------------------------------------------------
    ...
    s.set(1,3,"iii");
    s.set(1,4,"jjj");
    assert (s.get(0,0) == "aaa");
    assert (s.get(1,2) == "hhh");
    assert (s.get(1,4) == "jjj");
    t.print();
    std::cout << "set & get tests completed" << std::endl;
    ...
    row.push_back("abc");
    row.push_back("easy as a");
    row.push_back("one two three");
    row.push_back("abc");
    row.push_back("one two three");
    s.push_back_row(row);
    assert (s.numRows() == 3);
    ...
    col.push_back("and it's");
    col.push_back("sweet sweet");
    col.push_back("sweet victory yeah");
    s.push_back_column(col);
    assert (s.numRows() == 3);
    assert (s.numColumns() == 5);
    s.print();
    std::cout << "add column test completed" << std::endl;
    ...
    // example of the copy constructor
    Table<std::string> another1(s);
    // the structures initially look the same
    assert (s.numRows() == another1.numRows());
    assert (s.numColumns() == another1.numColumns());
    for (unsigned  i = 0; i < s.numRows(); i++) {
        for (unsigned  j = 0; j < s.numColumns(); j++) {
            assert (s.get(i,j) == another1.get(i,j));
        }
    }

    another1.set(1,2,"QQQ");
    assert (s.get(1,2) != another1.get(1,2));
    std::vector<std::string> row2;
    row2.push_back("AAA");
    row2.push_back("BBB");
    row2.push_back("SUPER");
    row2.push_back("SMASH");
    row2.push_back("BROTHERS");
    another1.push_back_row(row2);
    assert (s.numRows()+1 == another1.numRows());
    assert (s.numColumns() == another1.numColumns());
    s.print();
    another1.print();
    std::cout << "copy constructor test completed" << std::endl;
